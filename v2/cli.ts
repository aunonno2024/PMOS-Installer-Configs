...
import { PMOSInstallerConfig } from "../lib/schema";
  ...
  return Promise.all(
    files.map(file =>
      fs
        ...
        .then(obj => cb(obj as PMOSInstallerConfig, file as string))
    )
  ).then(r => r.flat());
};
...
program
  ...
  .description("Validate a PMOS Installer config YAML file.")
...
program
  ...
  .description(
    "Test PMOS Installer config Schema against YAML files. Files may specify 'expectedErrors' on the top level."
  )
...
program
  ...
  .description(
    "Ensure all files work with a specified version of the PMOS Installer. If no version is specified, the latest version is determined."
  )
  .argument("[glob...]", "file(s) to check", ["./v2/devices"])
  .option(
    "-v, --pmos-installer-version <version>",
